...
describe('Starting tests for healthrecords', function() {
  ...
  it('website title should be healthrecords', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('healthrecords');
    })
  });

  it('network-name should be healthrecords@3.0.0',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('healthrecords@3.0.0.bna');
    });
  });

  it('navbar-brand should be healthrecords',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('healthrecords');
    });
  });
