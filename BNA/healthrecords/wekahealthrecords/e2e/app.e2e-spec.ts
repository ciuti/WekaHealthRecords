...
describe('Starting tests for wekahealthrecords', function() {
  ...
  it('website title should be wekahealthrecords', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('wekahealthrecords');
    })
  });

  it('network-name should be healthrecords@4.0.0',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('healthrecords@4.0.0.bna');
    });
  });

  it('navbar-brand should be wekahealthrecords',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('wekahealthrecords');
    });
  });
    ...
    it('Request table should have 6 columns',() => {
      page.navigateTo('/Request');
      element.all(by.css('.thead-cols th')).then(function(arr) {
    ...
    it('CreateRequest component should be loadable',() => {
      page.navigateTo('/CreateRequest');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('CreateRequest');
      });
    });
